...
function validateNode(node, index, nodeIds, defaults, errors, structureOnly = false, valuesOnly = false) {
  ...
  // STRUCTURE: Validate required fields exist (HTTP nodes only)
  if (nodeType !== 'command') {
    if (!valuesOnly) {
      if (!node.method) {
        errors.push({
  ...
  if (nodeType !== 'command' && node.headers !== undefined) {
  ...
  if (nodeType !== 'command' && !structureOnly && node.body !== undefined) {

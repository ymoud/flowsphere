 ...
 */
function substituteVariables(input, context, substitutions = null, path = '') {
  if (typeof input !== 'string') {
    return input;
  }
  ...
  output = output.replace(guidRegex, (match) => {
    const uuid = generateUUID();
    if (substitutions) {
      substitutions.push({ original: match, value: uuid, type: 'dynamic-guid', path });
    }
    if (enableDebug) {
      console.error(`DEBUG: Replacing {{ $guid }} with ${uuid}`);
    }
  ...
  output = output.replace(timestampRegex, (match) => {
    if (substitutions) {
      substitutions.push({ original: match, value: timestamp, type: 'dynamic-timestamp', path });
    }
    if (enableDebug) {
      console.error(`DEBUG: Replacing {{ $timestamp }} with ${timestamp}`);
    }
  ...
  output = output.replace(varsRegex, (match, varName) => {
    ...
    if (substitutions) {
      substitutions.push({ original: match, value: value, type: 'variable', path });
    }
    if (enableDebug) {
  ...
  output = output.replace(inputRegex, (match, inputName) => {
    ...
    if (substitutions) {
      substitutions.push({ original: match, value: value, type: 'input', path });
    }
    if (enableDebug) {
  ...
  output = output.replace(responseRegex, (match, nodeId, fieldPath) => {
    ...
    if (substitutions) {
      substitutions.push({ original: match, value: value, type: 'response', path });
    }
    if (enableDebug) {
 ...
 * @param {SubstitutionContext} context - Substitution context
 * @param {Array} substitutions - Optional array to collect substitution records
 * @param {string} path - Current path in the object (for tracking location)
 * @returns {*} Object with all variables substituted
 */
function substituteInObject(obj, context, substitutions = null, path = '') {
  if (typeof obj === 'string') {
    return substituteVariables(obj, context, substitutions, path);
  }

  if (Array.isArray(obj)) {
    return obj.map((item, index) =>
      substituteInObject(item, context, substitutions, `${path}[${index}]`)
    );
  }
  ...
  if (obj && typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      result[key] = substituteInObject(value, context, substitutions, newPath);
    }
    return result;
  }
